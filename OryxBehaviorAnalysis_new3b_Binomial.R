...
#               Bayesian binomial regression.  Creating a model for each behavior.
...
# Could also plot the trace plots using the MCMCvis package
MCMCtrace(zm2)
...
par(mfrow=c(1,1))
hist(plogis(df1$mu.int),freq=FALSE,breaks=100, main = "Standing Head-Up (SHU)", xlab="Probability", ylim=c(0,100), col="light grey")
...
lines(density(plogis(df1$mu.int+df1$beta2)),lwd=2,col="red") # Category 3
...
# Investigate individual behaviors
...
hist(df1$beta2-df1$beta1,freq=FALSE,breaks=100, ylim=c(0,5),xlim=c(-2,2), main="Comparison Across Time Periods", xlab="Difference")
...
# Can't plot the first value...all zeros
for (i in 2:ncol(y)){
  # Plot histogram, eliminating burn-in
